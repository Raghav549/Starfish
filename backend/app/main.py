...
app=FastAPI(title="Starfish Fingerprint Engine",version="0.5.0")
...
@app.get("/health")
def health():
    return {"status":"ok","service":"starfish","version":"0.5.0"}
...
async def decode_image(file:UploadFile)->np.ndarray:
    ...
    if len(raw)>MAX_UPLOAD_BYTES:
        raise HTTPException(413,"IMAGE_TOO_LARGE: upload must be 4 MB or smaller.")
...
async def extract(file:UploadFile=File(...)):
    ...
    for name in ("enhanced","mask","skeleton","overlay"):
        ok=cv2.imwrite(str(OUTPUTS/f"{job}_{name}.png"),result[name])
        if not ok:
